...
/*
-criando a a estrutura do caixa, como vai funcionar sua interação com o usuario(aqui com funcionarios para venda).
...
int main(){
	...
	do{
		...
		do{
			...
			scanf("%d",&codigo_funcionario);
			//fazer a busca no banco de dados,se existe mesmo esse id,se sim ,adicionar ou incrementar os saldo de vendas do funcionario:saldo_vendas=saldo_vendas+quantidade_produto*preco_venda_do_produto e pode-se incrementar o total_vendas do funcionario com cada venda(sem muito valor prático).
