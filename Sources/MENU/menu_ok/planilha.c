...
void compras_realizadas(Compra *vetor_c,int tamanho_vetor,char *nome_arquivo){
	...
	for(i=0;i<tamanho_vetor;i++){
		fprintf(file_PTR,"Id da compra:,horario da compra:,dia da compra:,Produtos da compra:,preço do produto:,quantidade do produto vendida:\n");
		...
		for(j=0;j<(vetor_c+i)->total_produtos;j++){
			...
			fprintf(file_PTR,"%.2f,",((vetor_c+i)->produto+j)->preco_venda);
